...
A simple unikernel service that calls gemini api endpoints written in Rust

## Requirements

This service uses (openai schema) as its base, it can be configured to use any openai schema enabled service, along with the relevant api token
...
cd rust-ai-unikernel-gemini-service
...
```
./target/release/ai-unikernel-gemini-service
```
